...
class Barrage extends My_Controller {
	...
	public function index($op=''){
		if($op == 'json'){
	        ...
	        //总数量
		    $total = $this->mydb->get_nums('vod_barrage',$where,$like);
			//总页数
		    $pagejs = ceil($total / $per_page);
		    if($pagejs == 0) $pagejs = 1;
		    if($page > $pagejs) $page = $pagejs;
		    $limit = array($per_page,$per_page*($page-1));
		    $list = $this->mydb->get_select('vod_barrage',$where,'*','id desc',$limit,$like);
		    foreach($list as $k=>$v){
		    	...
		    	$list[$k]['duration'] = get_time($v['duration']);
			...
			$this->load->view('barrage/index.tpl');
		}
	}
	...
	public function del(){
		...
		$this->mydb->get_del('vod_barrage',$id);
		get_json('删除成功');
	}
}