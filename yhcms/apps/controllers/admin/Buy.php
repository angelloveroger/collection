...
class Buy extends My_Controller {
	...
	public function index($op=''){
		if($op == 'json'){
	        ...
	        //总数量
		    $total = $this->mydb->get_nums('user_vod_buy',$where,$like);
			//总页数
		    $pagejs = ceil($total / $per_page);
		    if($pagejs == 0) $pagejs = 1;
		    if($page > $pagejs) $page = $pagejs;
		    $limit = array($per_page,$per_page*($page-1));
		    $list = $this->mydb->get_select('user_vod_buy',$where,'*','id desc',$limit,$like);
		    foreach($list as $k=>$v){
				...
				$list[$k]['pic'] = getpic($rowv['pic']);
				$list[$k]['name'] = $rowv['name'];
				$list[$k]['jname'] = getzd('player','name',getzd('vod_zu','pid',$v['zid'])).'['.getzd('vod_ji','name',$v['jid']).']';
		    }
	        //记录数组
	        $data['count'] = $total;
	        $data['data'] = $list;
	...
	public function del(){
		...
		$this->mydb->get_del('user_vod_buy',$id);
		get_json('删除成功');
	}
}