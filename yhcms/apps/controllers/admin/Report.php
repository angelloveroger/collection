...
class Report extends My_Controller {
	...
	public function index($op=''){
		if($op == 'json'){
	        ...
	        //总数量
		    $total = $this->mydb->get_nums('comment_report',$where,$like);
			//总页数
		    $pagejs = ceil($total / $per_page);
		    if($pagejs == 0) $pagejs = 1;
		    if($page > $pagejs) $page = $pagejs;
		    $limit = array($per_page,$per_page*($page-1));
		    $list = $this->mydb->get_select('comment_report',$where,'*','id desc',$limit,$like);
		    foreach($list as $k=>$v){
		    	$list[$k]['addtime'] = date('Y-m-d H:i:s',$v['addtime']);
		    	if(date('Y-m-d',$v['addtime']) == date('Y-m-d')) $list[$k]['addtime'] = '<font color=red>'.$list[$k]['addtime'].'</font>';
		    	$list[$k]['pltext'] = getzd('comment','text',$v['did']);
		    }
	        //记录数组
	        $data['count'] = $total;
	        $data['data'] = $list;
			...
			$data['type'] = $this->myconfig['report'];
			$this->load->view('report/index.tpl',$data);
		}
	}
	...
	public function del(){
		...
		$this->mydb->get_del('comment_report',$id);
		get_json('删除成功');
	}
}