...
layui.use(['layer','form','element','table','laydate'],function() {
    ...
    //tps吸附提示
    var tpsindex = null;
    $('body').on('mouseenter', '.layui-input,.layui-textarea', function() {
        clearTimeout(tpsindex);
        var that = $(this);
        if(that.attr('placeholder') != undefined){
            tpsindex = setTimeout(function() {
                layer.tips(that.attr('placeholder'),that,{time:0,tips:1});
            },600);
        }
    });
    $('body').on('mouseleave', '.layui-input,.layui-textarea', function() {
        clearTimeout(tpsindex);
        layer.closeAll('tips');
    });
    //监听全局表单提交
...
;!function (win) {
        ...
        $.ajax({
            ...
            success: function(res){
                if(res.code == 1  && res.data) {
                    if(res.data.ver > config.ver && win.Admin.get_cookie('yhcms_ver') !== res.data.ver){
                        ...
                        parent.layer.open({
                            ...
                            btn3: function(index, layero) {
                                win.Admin.set_cookie("yhcms_ver", res.data.ver);
                            },
                            btn4: function(index, layero) {
                                win.Admin.set_cookie("yhcms_ver", res.data.ver, 30);
                            },
                            yes: function(index) {
                                layer.close(index);
                                parent.Admin.open(res.data.ver + '在线更新','/'+config.self+'/update?url='+res.data.zipurl+'&token='+res.data.uptoken,700,400);
                            },
                        });
                    }
                    $('.communique').html(res.data.communique);
                }else{
    ...
    //默认模版设置
    Admin.prototype.skin_init = function(_url,_dir) {
        if(_dir == null || _dir ==''){
            ...
            layer.confirm('确定要设置该模版为默认吗', {
                ...
                $.post(_url, {'dir':_dir}, function(res) {
                    if(res.code == 1){
                        layer.msg(res.msg,{icon: 1});
                        setTimeout(function() {
                            window.location.href = res.url;
    ...
    //默认模版设置
    Admin.prototype.skin_del = function(_url,_dir) {
        if(_dir == null || _dir ==''){
            layer.msg('请选择模版',{icon: 2,shift:6});
        }else{
            layer.confirm('确定要删除吗', {
    ...
    //修改密码
    Admin.prototype.set_pass = function(_link){
        layer.prompt({
            formType: 1,
            value: '',
            title: '请输入新密码',
            ...
            var tindex = layer.load();
            $.post(_link, {'pass':value}, function(res) {
                layer.close(tindex);
                if(res.code == 1){
                    layer.close(index);
                    layer.msg(res.msg,{icon: 1});
